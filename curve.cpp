...
Curve::Curve(std::vector<Coordinates_3d*> points, std::string name, Color* color)
{
	_points = points;
	_name = name;
	//_scn_points = _points;
	...
	if(((_points.size()-1) % 3) == 0){
		for( int i = 0; i < _points.size()-1; i = i+3){
			...
			for(double t = 0; t<=1;t = t+step )
			{
				...
				drawing_points.push_back(Coordinates_3d(x,y,0));
			}
			it = it+3;
		}
		/*  Clipping  */
...
bool Curve::too_far_away(Coordinates_3d c){
	if (c.get_x() < -10 || c.get_x() > 10 || c.get_y() > 10 || c.get_y() > 10 )
		return true;
	else
		return false;
}
bool Curve::is_inside(Coordinates_3d c){
