...
static void draw_object(GtkWidget *widget, cairo_t *cr, gpointer data){
	pair<Objeto*, UI*> obj_ui = *(pair<Objeto*, UI*>*)data;
	cout << "got her" << endl;
	string x = obj_ui.first->get_name();
	cout << "got her" << endl;
	obj_ui.second->real_draw(obj_ui.first, cr, widget);
}
...
static void move_up_callback (GtkWidget *widget, gpointer data){
	cout << "KAPPA PRIDE" << endl;
	pair<int, int> a = *((pair<int, int>*)data);
	cout << a.first << endl;
}
...
void UI::draw(Objeto* obj){
	pair<Objeto*, UI*> stuff(obj, this);
	g_signal_connect (_canvas, "draw", G_CALLBACK (draw_object), &stuff);
}
void UI::real_draw(Objeto* obj, cairo_t *cr, GtkWidget* widget){
	...
	/* Getting the window bounds coordinates*/
	cout << "got here" << endl;
	//Coordenadas* window_min = _mundo->get_window_min();
	//Coordenadas* window_max = _mundo->get_window_max();
	Coordenadas* window_min = new Coordenadas(0,0);
	Coordenadas* window_max = new Coordenadas(500,500);
	cout << "got here" << endl;
