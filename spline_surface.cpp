...
Spline_Surface::Spline_Surface(std::vector<Coordinates *> points, std::string name, Color *color, bool blending)
...
void Spline_Surface::forward_diff(cairo_t *cr, Coordinates win_min, Coordinates win_max,
...
void Spline_Surface::blending_function(cairo_t *cr, Coordinates win_min, Coordinates win_max,
                                       Coordinates vp_min, Coordinates vp_max) {
    double s = 1.0 / 16;
    double t = 1.0 / 16;
    ...
    if (((_points.size() - 1) % 15) == 0) {
        ...
        for (int i = 15; i < _points.size(); i++, it++) {
