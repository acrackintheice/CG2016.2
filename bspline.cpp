...
BSpline::BSpline(std::vector<Coordinates_3d*> points, std::string name, Color* color)
{
	_points = points;
...
vector<Edge> BSpline::clip(){
	vector<Coordinates_3d> drawing_points;
	vector<Edge> edges;
	...
	/* Now for each curve */
	vector<Coordinates_3d*>::iterator it = _points.begin();
	for (int i = 3; i < _points.size(); i++,it++)
	{
		/* Creating the Geometric matrixes in order to calculate the Cs */
		Matriz4x1 gx = Matriz4x1((*it)->get_x_scn(),(*(it+1))->get_x_scn(), (*(it+2))->get_x_scn(), (*(it+3))->get_x_scn());
		Matriz4x1 gy = Matriz4x1((*it)->get_y_scn(),(*(it+1))->get_y_scn(), (*(it+2))->get_y_scn(), (*(it+3))->get_y_scn());
		/* Calculating the Cs */
		...
		std::vector<Coordinates_3d> some_points = getFwdDiffPoints(1/deltao,dx,dy);
	...
	{
		vector<Coordinates_3d>::iterator it2 = drawing_points.begin();
		for (int i = 0; i < drawing_points.size()-1; ++i,it2++){
			Coordinates_3d p1 = *(it2);
			Coordinates_3d p2 = *(it2+1);
			Line l = Line(new Coordinates_3d(p1.get_x(), p1.get_y(),0), new Coordinates_3d(p2.get_x(), p2.get_y(),0),"",new Color(1,1,1,1));
			;
			vector<Edge> clipped = l.clip();
			if(clipped.size() > 0){
				Coordinates_3d* p1 = clipped[0].get_p1();
				Coordinates_3d* p2 = clipped[0].get_p2();
...
std::vector<Coordinates_3d> BSpline::getFwdDiffPoints(double n, Matriz4x1 dx, Matriz4x1 dy)
{
	double x       = dx.get(0);
	...
	std::vector<Coordinates_3d> some_points;
	for (int i = 0; i < n; ++i)
	{
		x       = x + deltaX;
		...
		some_points.push_back(Coordinates_3d(x,y,0));
	}
	return some_points;
}
bool BSpline::too_far_away(Coordinates_3d c)
{
...
bool BSpline::is_inside(Coordinates_3d c)
{
